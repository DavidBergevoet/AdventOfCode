...
bool PathContainsSmallTwice(const std::vector<Node>& rPath)
{
    for(size_t i = 0; i < rPath.size(); ++i)
    {
        if(!rPath[i].GetBigNode() && rPath[i].GetSymbol() != "start")
        {
            for(size_t j = 0; j < rPath.size(); ++j)
            {
                if(i != j && rPath[i].GetSymbol() == rPath[j].GetSymbol())
                    return true;
            }
        }
    }
...
void FindPathInternal(Node* pNode, Node* pEndNode, const std::vector<Node>& rPath, std::vector<std::vector<Node>>& rTotalPaths)
{
    ...
    else if(!pNode->GetBigNode() && IsInPath(*pNode, pathCopy) && PathContainsSmallTwice(pathCopy))
        return;
    pathCopy.push_back(*pNode);

    for(Node* pAdjacent : pNode->GetAdjacent())
    {
        if(pAdjacent->GetSymbol() != "start")
        {
...
int main(int argc, char const *argv[])
{
    ...
    // std::vector<Node> testPath;
    // testPath.push_back(Node("b", false));
    // testPath.push_back(Node("A", true));
    // testPath.push_back(Node("b", false));
    // bool result = PathContainsSmallTwice(testPath);
