...
struct Monkey{
    ...
    uint m_divisible;
    uint m_trueMonkeyNumber;
    uint m_falseMonkeyNumber;

    uint m_inspections = 0;
};
...
int main(int argc, char const *argv[])
{
    ...
    for(size_t round = 0; round < 20; ++round)
    {
        for(size_t i = 0; i < monkeyList.size(); ++i)
        {
            ...
            while(monkeyList[i].m_items.size() != 0)
            {
                ...
                {
                    monkeyList[monkeyList[i].m_trueMonkeyNumber].m_items.push_back(monkeyList[i].m_items[0]);
                }
                else
                {
                    monkeyList[monkeyList[i].m_falseMonkeyNumber].m_items.push_back(monkeyList[i].m_items[0]);
                }
    ...
    uint32_t highestMonkeys[2] = {0, 0};
    for(const Monkey& rMonkey : monkeyList)
    {
        uint32_t currentLowest = (highestMonkeys[0] < highestMonkeys[1]) ? highestMonkeys[0] : highestMonkeys[1];
        uint lowestIndex = (highestMonkeys[0] < highestMonkeys[1]) ? 0 : 1;
