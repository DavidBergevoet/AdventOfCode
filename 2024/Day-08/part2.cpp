    ...
    for (int i = 0; i < rAntenna.second.size(); ++i)
    {
      ...
      for (int j = 0; j < rAntenna.second.size(); ++j)
      {
        if (i != j)
        {
          ...
          {
            if (rPoint == rAntenna.second[i])
              containsA = true;
            if (rPoint == rAntenna.second[j])
              containsB = true;
          }
          if (!containsA)
            antiNodes.push_back(rAntenna.second[i]);
          if (!containsB)
            antiNodes.push_back(rAntenna.second[j]);
          ...
          while (antiNode.first >= 0 && antiNode.first < height && antiNode.second >= 0 && antiNode.second < width)
          {
            ...
            antiNode = MakePoint(antiNode.first + rowDiff, antiNode.second + colDiff);
          }
        }
      }
    }
